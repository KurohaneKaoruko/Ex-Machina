...
生成成功后，共享内容直接位于仓库根目录的 `skills/`、`agents/`、`commands/`、`hooks/`、`codex/`、`trae/` 等目录；平台安装脚本仍位于根目录 `scripts/`。
...
旧的嵌套产物目录 `./exmachina` 已从当前架构中移除。现在唯一有效的共享内容路径就是根目录下的 `skills/`、`agents/`、`commands/`、`hooks/`、`codex/`、`trae/` 等目录。
