...
- `连结体` 是团队概念，不是单个智能体。
- 单个智能体必须写成 `xx连结指挥体`。
- 一个 `xx连结体` 由 `xx连结指挥体 + 若干对应子个体` 组成。

例如：

- `研究连结指挥体` 是单个智能体。
- `研究连结体` 则表示“研究连结指挥体 + 溯源体 + 比对体 + 假设体 + 证据体 + 反证体 ...”这一整支团队。
...
现在可以直接把仓库的 `exmachina/skills/` 接入本地 Codex 技能库，并把 `exmachina/agents/` 同步到 `~/.codex/agents/`。

安装文档：

- 仓库内：[`exmachina/codex/INSTALL.md`](exmachina/codex/INSTALL.md)
- Raw URL：`https://raw.githubusercontent.com/KurohaneKaoruko/Ex-Machina/main/exmachina/codex/INSTALL.md`
...
| 平台 | 安装文件位置 | 参考文档 |
|------|-------------|---------|
| OpenAI Codex | `scripts/` + `exmachina/skills/` + `exmachina/agents/` + `exmachina/codex/` | [exmachina/codex/INSTALL.md](exmachina/codex/INSTALL.md), [exmachina/codex/INSTALL.en.md](exmachina/codex/INSTALL.en.md), [exmachina/codex/README.md](exmachina/codex/README.md), [exmachina/codex/README.en.md](exmachina/codex/README.en.md) |
| Trae | `exmachina/trae/` | [trae/INSTALL.md](exmachina/trae/INSTALL.md), [trae/INSTALL.en.md](exmachina/trae/INSTALL.en.md) |
| Cursor | `exmachina/cursor/` | 规则产物已生成，安装文档待继续细化 |
| Claude Code | `exmachina/claude-plugin/` | 插件清单已生成，安装与运行时集成待继续细化 |
| VS Code | `exmachina/vscode/` | Prompt 与 instructions 产物已生成 |
| Kiro | `exmachina/kiro/` | Skill 与 steering 产物已生成 |
...
生成成功后，平台产物位于 `exmachina/` 下；Codex 安装脚本位于根目录 `scripts/`，便于直接查找和执行。

## 产物层说明

`src/` 是唯一源码层，负责维护真正需要人工编辑的内容。
...
`exmachina/` 是最终给外部软件安装、加载或引用的产品目录。
...
Kiro 使用面，包含 Skill 与 steering 入口。

### `exmachina/vscode/`

VS Code 风格的 prompt / instructions 使用面。
...
- 更强的运行时路由能力
- 更完整的自动评测回路
- Cursor / Claude Code / VS Code 等平台的安装细节继续细化
- 更稳定的场景基准与回归机制
